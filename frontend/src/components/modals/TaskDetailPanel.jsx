import { Check, Trash2, Workflow, X, XCircle } from "lucide-react";
...
export function TaskDetailPanel() {
  const { addSubtask, blocks, deleteBlock, deleteSubtask, pages, selectedTaskId, setActivePage, setSelectedTask, setTaskDependencies, toggleTask, toggleFailTask, updateSubtask, updateTask } = useAppStore();
  ...
  return (
    <aside ref={panelRef} className="task-detail-panel flex flex-col fixed inset-y-0 right-0 z-30 w-[430px] max-w-full border-l-[3px] border-black bg-[#fff7e8] shadow-[-4px_0_0_#111] dark:border-[#1e232a] dark:bg-[#0c0e11] dark:shadow-[-3px_0_0_#000]">
      ...
      <div className="grid content-start flex-1 gap-4 overflow-auto p-5 pt-2">
        ...
        {!isVirtual && (
          <button
            className="nb-button danger w-full flex items-center justify-center gap-2"
            onClick={() => {
              void deleteBlock(task.id);
              setSelectedTask(undefined);
            }}
            type="button"
          >
