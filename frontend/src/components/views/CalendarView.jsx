...
export function CalendarView({ searchQuery }) {
  ...
  return (
    <div className="bento-grid">
      <div className="bento-card span-12 flex items-center justify-between bg-[#21caff] p-4 text-black">
      ...
      <section className="bento-card span-8 grid grid-cols-7 gap-2 p-4 max-sm:gap-1">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div className="text-center text-xs font-black uppercase text-stone-700 dark:text-slate-200" key={day}>{day}</div>)}
        {days.map((day) => {
          const key = day.toISOString().slice(0, 10);
          const dayTasks = tasks.filter((task) => toDateInput(task.metadata.deadline) === key);
          const inMonth = day.getMonth() === cursor.getMonth();
          return (
            <button className={clsx("min-h-24 rounded-lg border-[3px] border-black p-2 text-left font-black shadow-[3px_3px_0_#111] transition hover:-translate-y-1 hover:shadow-[5px_5px_0_#111] dark:border-white dark:shadow-[3px_3px_0_#fff] dark:hover:shadow-[5px_5px_0_#fff] max-sm:min-h-16", selectedDay === key ? "bg-[#ffdc4a] text-black" : "bg-white hover:bg-[#fff1b8] dark:bg-[#202020] dark:hover:bg-[#2b2b2b]", !inMonth && "opacity-45")} key={key} onClick={() => setSelectedDay(key)} type="button">
              <span className="text-sm">{day.getDate()}</span>
              <div className="mt-2 flex flex-wrap gap-1">
                {dayTasks.slice(0, 4).map((task) => <span className={clsx("h-3 w-3 rounded-sm border border-black", priorityDot(task.metadata.priority))} key={task.id} />)}
              </div>
      ...
      <aside className="bento-card span-4 bg-[#ffdc4a] p-4 text-black">
        ...
        <div className="grid gap-2">
          {selectedTasks.length ? selectedTasks.map((task) => <button className="nb-button justify-start bg-white text-left" key={task.id} onClick={() => setSelectedTask(task.id)} type="button">{task.content.title}</button>) : <p className="rounded-lg border-[3px] border-black bg-white p-4 text-sm font-black shadow-[4px_4px_0_#111]">No tasks scheduled.</p>}
        </div>
      </aside>
    </div>
  );
